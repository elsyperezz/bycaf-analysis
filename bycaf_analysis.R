...
# Frequency of area of residence --------------------------------
df_residence <- bycaf_data %>%
  group_by(Place_Of_Residence_A4) %>%
  summarize(Total = n()) %>%
  arrange(Total)
head(df_residence, 5)

# Frequency of visit frequency --------------------------------
df_visit <- bycaf_data %>%
...
## -------- Calculate summaries (av, min, max) ------------------------
# Summary of age of respondents --------------------------------
mean(bycaf_data$Age_A1, na.rm = TRUE)
min(bycaf_data$Age_A1, na.rm = TRUE)
max(bycaf_data$Age_A1, na.rm = TRUE)

# Summary of length of time living in the area --------------------------------
...
# Mosaic Plot of use by area of residence --------------------------------
ggplot(b5map_data) +
...
# Mosaic plot of appreciation by occupation --------------------------------
